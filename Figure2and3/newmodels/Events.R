dosingevents<-function(opioid_dose, opioid_time=0, naloxone_dose=4000000, naloxone_time,naloxone_doseN=0, gap){
	#naloxone_doses can only be 0,1,2,3,4,5
	...
	if(naloxone_doseN == 1){
		...
		eventdata<-rbind(eventdata, data.frame(var="D",time=c(naloxone_time, naloxone_time+gap, naloxone_time+gap+gap),value=c(rep(naloxone_dose,2), .85*naloxone_dose),method="add"))
		...
		eventdata<-rbind(eventdata, data.frame(var="D",time=c(naloxone_time, naloxone_time+gap),value=c(2*naloxone_dose,2*.85*naloxone_dose),method="add"))
	}
