...
#define timeout parms[101]
#define starttime parms[102]
...
double* calc_intermediate(double im11,double im12,double im13,double im14,double im15,double im16,double im18,double im19,double im25,double im26,double im27){
	...
	//double i_Qb = Cim28*Qb0*(1+Cim27*(im18+im19))*im25;
	//double i_Qt = Qt0*(1+Cim28*rou*im19)*im26;
...
void derivs (int *neq, double *t, double *y, double *ydot, double *yout, int *ip){
	...
	// double koutC=intermediate_list[23];		double k12C = intermediate_list[24];
	// double k13C=intermediate_list[25];		double k21C=intermediate_list[26];
	// double k31C=intermediate_list[27];		double VPC=intermediate_list[28];
	...
	//  if (CA_delay_o2<=imtime_L && P_a_o2>tsh_value_o2 && CA_o2_happen==1) {
	//  	Cim26_e_o2 = -rcov*Cim26*.5;
	//  	Cim26_e_co2 = -rcov*Cim26*0.5;
	//
	//  }
...
//	if(Q<Q_0){
	...
	ydot[3] = (1.00/V1C)*((1*kin*y[2])-(kout2*y[3]));
	...
	ydot[23] =0; /* FIV */
	ydot[24] =0; /* counter*/
	//	printf("%f,%f,%f,%f,%f\n", ydot[12], 1.00/V_A*(Venti*(y[17] - y[12]) + lumbda*(Qb+Qt)*(1-s1)*(C_V_o2 - C_e_o2)),Venti*(y[17] - y[12]) + lumbda*(Qb+Qt)*(1-s1)*(C_V_o2 - C_e_o2), Venti*(y[17] - y[12]) , lumbda*(Qb+Qt)*(1-s1)*(C_V_o2 - C_e_o2));
	//printf("%f", Cim26_e);
	...
	//yout[9]=history;
	//yout[10]=historyc;
	//yout[11]=Clag_P_B_co2;
	//yout[12]=P_B_co2;
	//yout[13]=Plag_C_B_co2;
	//yout[14]=Plag_P_a_co2;
	//Outputs
//	yout[0]=Venti*60/0.66;
//	yout[1]=residualWakefulnessDrive;
//	yout[2]=chemoreflex_drive;
//	yout[3]=Qb*60;
//	yout[4]=Qt*60;
//	yout[5]=P_a_o2;
//	yout[6]=P_a_co2;
//	yout[7]=P_B_o2;
//	yout[8]=P_B_co2;
//	yout[9]=y[5]; /* opioid plasma */
//	yout[10]=y[3]; /* antagonist plasma */
//	yout[11]=y[8]*1e-6; /* opioid effect site */
//	yout[12]=y[4]*1e-6; /* antagonist effect site */
//	yout[13]=y[9]; /* opioid bound receptor fraction */
//	yout[14]=y[10]; /* antagonist bound receptor fraction */
//	yout[15]=Q*60;//Qb*60+Qt*60; // Total blood flow
//	yout[16]=koutC;
//	yout[17]=k12C;
//	yout[18]=k13C;
//	yout[19]=k21C;
...
//	yout[23]=V1C;
	...
	//yout[22]=Venti;
	//yout[23]=Plag_f_pc;
	//yout[24]=Plag_P_a_co2;
	//yout[25]=Plag_P_a_co2;
	//yout[26]=Plag_P_a_o2;
	//yout[27]=psai_co2;
	//yout[28]=psai_o2;
	//yout[29]=Qb;
	//yout[30]=P_B_o2;
	//yout[31]=Clag_P_B_co2;
	//yout[32]=W - Kf;
	...
	yout[31]=Clag_P_B_co2;
	yout[32]=W - Kf;
	yout[33]=arterialOxygenSaturation;
}
