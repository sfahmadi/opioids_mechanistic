EmaxfVP2 <- function(ind){
	
	print("VP2")
	ind_Selected=ind[c("k1","PP1","PP3","P2","Wmax")] #VP2
	names(ind_Selected)=c("k1","P1","P3","P2","Wmax") #just adjust name
	nptarget=match(names(truepar), names(ind_Selected), nomatch=0)
	truepar[nptarget!=0]<- ind_Selected[nptarget]
truepar=unlist(truepar)

	patient_case="Chronic" #later combine Naive and chronic
...
truepar["A1"]=0; truepar["B1"]=0; #so that CAR won't change
